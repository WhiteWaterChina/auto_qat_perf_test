...
def test_openssl(dir_result):
    ...
    for item_qat_status in output_status_qat:
        ...
        if qat_state_list is not None:
            ...
            if qat_device_state.strip() == 'up':
                ...
                sys.exit(1)
    filename_result_qat_on =  os.path.join(dir_result, "result_openssl_qat_on.txt")
    ...
    test_qat_off = subprocess.Popen(["/etc/init.d/qat_service", "stop"])
    test_qat_off.wait()
    ...
    for item_qat_status in output_status_qat:
        ...
        if qat_state_list is not None:
            ...
            qat_device_bus = qat_state_list.groups()[1]
            qat_device_state = qat_state_list.groups()[2]
            if qat_device_state.strip() == 'down':
                print "qat_dev%s bus_number:%s is %s successfully!" % (qat_device_num, qat_device_bus, qat_device_state)
            else:
                print "qat_dev%s bus_number:%s is %s! Please check!" % (qat_device_num, qat_device_bus, qat_device_state)
                sys.exit(1)
    filename_result_qat_off = os.path.join(dir_result, "result_openssl_qat_off.txt")
...
def filter_openssl(filename_openssl_result, list_policy_filter):
    ...
    for index_policy, item_policy in enumerate(list_policy_filter):
...
def plot_openssl(list_policy_first, list_block_first, dir_result):
    policy_block_data_list = []
    for item_policy_first in list_policy_first:
        for item_block in list_block_first:
            data_item = item_policy_first + " " + item_block
            policy_block_data_list.append(data_item)
    openssl_qat_on = filter_openssl(os.path.join(dir_result, "result_openssl_qat_on.txt"), list_policy_filter)
    openssl_qat_off = filter_openssl(os.path.join(dir_result, "result_openssl_qat_off.txt"), list_policy_filter)
...
def qat_cpa_test(dir_result):
    path_file_result = os.path.join(dir_result, "qat_cpa_result.txt")
    ...
    filename_cpa_result = os.path.join(dir_result, "result_cpa_test.xlsx")
